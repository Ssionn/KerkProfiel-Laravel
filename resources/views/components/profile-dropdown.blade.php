...
<button id="userDropdown" data-dropdown-toggle="toggleUserDropdown" data-dropdown-placement="top"
    ...
    <div class="flex flex-col items-start" :class="open ? 'ml-2' : ''">
        <span x-show="open" class="text-sm font-medium">{{ auth()->user()->username }}</span>
        <span x-show="open" class="text-xs text-gray-500">{{ auth()->user()->team->name ?? 'Geen team' }}</span>
    </div>
</button>
    ...
    <ul class="py-[2px] px-[2px] text-sm text-gray-700" aria-labelledby="userDropdownList">
        ...
        <li>
            <form id="logout-form" action="{{ route('logout') }}" method="POST" class="w-full">
                ...
                <button type="submit" class="hover:bg-sky-blue hover:text-white block w-full text-start px-4 py-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-blue focus-visible:ring-opacity-75 rounded-lg">
                    {{ __('Logout') }}
                </button>
            </form>
        </li>
